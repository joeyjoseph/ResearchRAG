...
- For the first version of the tool we picked plain txt files, as the way both LLMs and scripts read them is clear and consistent. Markdown files are proabably fine, but the characters they use may add fragility to some scripts or tools. Maybe not. Try to pick one and stick with it. The less you have to manage the better.
- Note the script that will automatically add to the database converts everything to txt, so you'll need to change it if you prefer md files.
...
- Creat a directory structure. You'll see how we did it in the Install guide, but each project is different. Best to decided on this before you begin. For example, if you don't have, and don't anticipate dealing with interviews, then don't make an `Interviews/` folder.
- If you have files that are already in the correct format, place them in their appropriate folders within Corpus. If you have some that need converting, place them in the `Add To Corpus/` folder.
